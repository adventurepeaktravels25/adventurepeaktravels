      ...
      <header className="sticky top-0 z-40 backdrop-blur-md bg-background/85 border-b border-border">
        <div className="max-w-7xl mx-auto px-4 md:px-6 h-16 flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center gap-1">
            {NAV.map(n => (
              <Link key={n.to} href={n.to} className="px-4 py-2 rounded-lg text-sm font-medium transition-colors hover:bg-secondary/60">
                {n.label}
              </Link>
            ))}
          ...
          <details className="relative md:hidden group">
            ...
            <div className="absolute right-0 top-full mt-2 w-56 rounded-2xl border border-border bg-background p-2 shadow-soft">
              {NAV.map(n => (
                <Link key={n.to} href={n.to} className="block rounded-xl px-3 py-2.5 text-sm font-medium hover:bg-secondary">
                  {n.label}
                </Link>
              ))}
            </div>
