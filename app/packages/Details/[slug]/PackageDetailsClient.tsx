...
import { formatCurrency, type CatalogueItem, type CataloguePackageTier, type CatalogueServiceDetails } from "@/lib/api/catalogue";

export function PackageDetailsClient({ item }: { item: CatalogueServiceDetails | null }) {
  if (!item) {
    return (
      <section className="max-w-3xl mx-auto px-6 py-20 text-center">
        <p className="text-sm font-semibold uppercase tracking-[0.3em] text-primary">Package unavailable</p>
        <h1 className="mt-4 text-3xl md:text-4xl font-bold">We could not load this package right now.</h1>
        <Link href="/packages" className="mt-8 inline-flex items-center gap-2 rounded-lg bg-primary px-5 py-3 text-sm font-semibold text-primary-foreground hover:bg-primary/90">
          Back to packages
        </Link>
      </section>
    );
  ...
  const whatsappMessage = encodeURIComponent(
    `Hi! I'm interested in the ${item.name}${selectedTier ? ` - ${selectedTier.name} (${formatCurrency(selectedTier.price)})` : ""}. Please share itinerary, inclusions, and booking details.`,
  );
  ...
  return (
    <>
      <section className="max-w-7xl mx-auto px-4 sm:px-6 pt-6 md:pt-8">
        ...
        <Link href="/packages" className="inline-flex items-center gap-2 text-sm font-medium text-muted-foreground hover:text-primary">
        ...
        <div className="grid lg:grid-cols-[1.35fr_0.95fr] gap-6 lg:gap-8 items-start">
          <div className="space-y-4">
            ...
            <div className="rounded-2xl border border-border bg-card p-5 md:p-6 shadow-card">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <h2 className="text-2xl font-bold">Package Description</h2>
                </div>
                <div className="flex items-center gap-2 text-sm text-muted-foreground shrink-0">
                  <Clock3 className="h-4 w-4" />
                  <span>{selectedTier?.duration ?? "Package"}</span>
                </div>
              </div>
          ...
          <aside className="lg:sticky lg:top-24">
            <div className="rounded-3xl border border-border bg-card p-5 md:p-6 shadow-card">
              ...
              <p className="mt-1 text-sm text-muted-foreground">Switch between available package options for this service.</p>
                ...
                {(item.packages ?? []).map(pkg => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <div className="mt-3 flex flex-wrap gap-2">
                        <span className="inline-flex items-center rounded-full bg-secondary/70 px-2.5 py-1 text-[10px] font-medium text-foreground">{pkg.type}</span>
          ...
          <div className="mt-6 grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-5">
            {related.map((entry: CatalogueItem) => (
              <article key={entry._id} className="overflow-hidden rounded-2xl border border-border bg-card shadow-sm">
                <Link href={`/packages/Details/${entry.slug}`} className="block">
